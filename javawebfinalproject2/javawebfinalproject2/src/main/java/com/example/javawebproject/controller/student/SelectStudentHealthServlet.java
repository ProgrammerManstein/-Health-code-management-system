...
@WebServlet(name = "SelectStudentHealthServlet", value = "/SelectStudentHealthServlet")
public class SelectStudentHealthServlet extends HttpServlet {
    @Override
            ...
            if(studentBaseDateArrayList.size()==0){//不是num
                ...
                if(studentBaseDateArrayList.size()==0){//通过name和num查询都找不到结果
                    ...
                    if(studentBaseDateArrayList.size()==0){//通过学院查找
                        ...
                        if(studentBaseDateArrayList.size()==0){//通过专业查找
                            studentBaseDateArrayList=studentDao.findByClass(Class);
                            if(studentBaseDateArrayList.size()==0){//通过班级查找
                                studentBaseDateArrayList=studentDao.findAllStudent();
                                session.setAttribute("studentList",studentBaseDateArrayList);
                                response.sendRedirect(request.getContextPath()+"/SelectStudentAndTeacherHealth.jsp");
                                return;
                            }
