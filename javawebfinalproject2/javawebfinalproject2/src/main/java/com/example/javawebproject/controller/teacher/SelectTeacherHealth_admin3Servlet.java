...
@WebServlet(name = "SelectTeacherHealth_admin3Servlet", value = "/SelectTeacherHealth_admin3Servlet")
public class SelectTeacherHealth_admin3Servlet extends HttpServlet {
    @Override
            ...
            if(teacherBaseDateArrayList.size()==0){//不是num
                ...
                if(teacherBaseDateArrayList.size()==0){//通过name和num查询都找不到结果
                    ...
                    if(teacherBaseDateArrayList.size()==0){//通过学院查找
                        ...
                        response.sendRedirect(request.getContextPath()+"/SelectTeacherAndStudentHealth_admin3.jsp");
                        return;
                    }
